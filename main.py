...
@app.get("/debug/download-active-listings-csv")
def download_active_listings_csv(
    filename: str = Query(
        "debug_active_listings.csv",
    ...
    """
    DEBUG ENDPOINT: Download the Active Listings CSV file generated by the export endpoint.
    
    This allows you to download the CSV file directly through your browser.
    First run /debug/export-active-listings-csv to generate the file,
    then use this endpoint to download it.
    """
    ...
    if not os.path.exists(filepath):
        raise HTTPException(
            status_code=404,
            detail=f"CSV file '{filename}' not found. Run /debug/export-active-listings-csv first to generate it."
        )
