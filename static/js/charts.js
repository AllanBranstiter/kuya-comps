...
// ============================================================================
// COMPARISON BEESWARM CHART
// ============================================================================

/**
 * Draw comparison beeswarm chart for multiple cards
 * @param {Array} cardResults - Array of card result objects
 */
function drawComparisonBeeswarm(cardResults) {
    const canvas = document.getElementById("comparisonBeeswarmCanvas");
    if (!canvas || !cardResults || cardResults.length === 0) {
        console.error('[CHART] Canvas not found or no card results');
        return;
    }
    ...
    const cardColors = [
        { fill: 'rgba(0, 122, 255, 0.3)', stroke: 'rgba(0, 122, 255, 0.9)', solid: 'rgb(0, 122, 255)' },
        { fill: 'rgba(255, 59, 48, 0.3)', stroke: 'rgba(255, 59, 48, 0.9)', solid: 'rgb(255, 59, 48)' },
        { fill: 'rgba(52, 199, 89, 0.3)', stroke: 'rgba(52, 199, 89, 0.9)', solid: 'rgb(52, 199, 89)' }
    ];
    ...
    cardResults.forEach(result => {
        if (result.fmvLow != null) globalMin = Math.min(globalMin, result.fmvLow);
        if (result.fmvHigh != null) globalMax = Math.max(globalMax, result.fmvHigh);
    });
    ...
    const xScale = (price) => {
        if (priceRange === 0) return width / 2;
