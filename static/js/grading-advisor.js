...
const GradingAdvisor = (function() {
    ...
    function renderResults(response) {
        ...
        // This also sets response._scenarioWarning for use below
        ...
        if (response.distribution) {
            html += renderPopulationChart(response.distribution, response.matrix);
        }
        
        // Build warnings array with scenario warning included
        const allWarnings = [];
        
        // Add scenario warning first (generated by createDecisionSummaryCard)
        if (response._scenarioWarning) {
            allWarnings.push(response._scenarioWarning);
        }
        
        // Add existing warnings from API response
        ...
        if (allWarnings.length > 0) {
            html += renderWarnings(allWarnings);
        }
    ...
    /**
     * Generate a scenario-based plain English warning
     * @param {Array} grades - Array of grade objects with profit_loss
     * @param {number|null} breakEvenGrade - The minimum grade needed to profit
     * @param {number} maxLoss - The largest potential loss (negative number)
     * @returns {string} Warning message
     */
    function generateScenarioWarning(grades, breakEvenGrade, maxLoss) {
        if (!breakEvenGrade) {
            return "⚠️ No profitable grades found. Grading this card will likely result in a loss.";
        }
        
        const gradeWord = breakEvenGrade === 10 ? 'PSA 10' : `PSA ${breakEvenGrade}+`;
        const belowGrade = breakEvenGrade - 1;
        
        return `Most graded copies receive ${gradeWord}. If you hit ${gradeWord}, you profit. If you fall to PSA ${belowGrade} or below, you lose up to ${formatCurrency(Math.abs(maxLoss))}.`;
    }
    
    /**
     * Create Decision Summary Hero Card
     * Provides clear, actionable guidance on whether to grade a card
     * Uses a Confidence Score system based on break-even grade and high grade population
    ...
    function createDecisionSummaryCard(data) {
        ...
        // 1. Calculate success probability (sum of probabilities for profitable grades)
        let successProb = 0;
        grades.forEach(g => {
        ...
        const psa9Pop = gradePercentages['9'] || 0;
        const highGradePopPercent = psa10Pop + psa9Pop;
        
        // 4. Calculate max loss for scenario warning
        let maxLoss = 0;
        grades.forEach(g => {
            if (g.profit_loss < maxLoss) {
                maxLoss = g.profit_loss;
            }
        });
        ...
        // 7. Format break-even display with "or Higher" format
    ...
    /**
     * Render the population distribution chart with horizontal bars (PSA 10 to PSA 1, left to right)
     * @param {Object} distribution - Population data { grade_percentages, total_population, rarity_tier }
     * @param {Object} matrix - Grade matrix data with is_profitable flags
     * @returns {string} HTML string
     */
    function renderPopulationChart(distribution, matrix) {
        ...
        // Sort grades descending (10 to 1) for left-to-right display
        ...
        for (const grade of grades) {
            ...
            if (matrix && matrix[grade.toString()]) {
                isProfitable = matrix[grade.toString()].is_profitable || false;
            }
        ...
        return `
            <div class="result-module" style="width: 100%;">
                ...
                <div class="population-chart-horizontal">
                    ${columnsHtml}
                </div>
            </div>
        `;
